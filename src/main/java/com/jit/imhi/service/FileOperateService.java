...
public class FileOperateService {
    ...
    @Autowired
    FileOperateService(HistoryMessageMapper historyMessageMapper, GroupuserMapper groupuserMapper) {
    ...
    public HistoryMessage authenicationFileOwner(Integer MessageId, Integer userId) {
        ...
        if (historyMessage == null) {
            System.out.println("authenication这里已经为空了");
            return null;
        }

        System.out.println("  " + historyMessage.getUserFromId());
        System.out.println("  " + userId);
        System.out.println("  " + historyMessage.getToId());
        if (historyMessage.getUserFromId().equals(userId) || historyMessage.getToId().equals(userId) ||
                (groupuserMapper.selectByPrimaryKey(userId, historyMessage.getToId()) != null)) {

            // 查找这条消息是否属于此用户
            System.out.println("authenica 成功进入");
            System.out.println("  " + historyMessage.getUserFromId());
            System.out.println("  " + userId);
            System.out.println("  " + historyMessage.getToId());
        ...
        System.out.println("authenica 成功进入NM");
        return null;// 并不是
    }
